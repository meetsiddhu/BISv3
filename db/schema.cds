...
// ── R6 UNIFICATION — one READ model over BOTH restriction masters ─────────────
// The system intentionally keeps TWO restriction WRITE masters (locked rule 1,
// additive-only — neither table may be removed): `Restrictions` (maintained by
// the Restrictions Fiori app, mass upload and mass edit) and `BridgeRestrictions`
// (maintained from the Bridges register object-page tab). Before v3.13.x every
// read surface picked ONE master, so the Restrictions Dashboard ALP, the
// operational dashboard KPIs, prioritisation restrictionFlag and the
// postingStatus derivation could disagree by construction (council finding R6).
...
entity UnifiedRestrictions as
  select from Restrictions {
    key ID,
        'Restrictions'         as sourceMaster  : String(20),
        bridge.ID              as bridge_ID     : Integer,
        bridge.bridgeId        as bridgeId      : String(40),
        bridge.bridgeName      as bridgeName    : String(111),
        bridge.transportMode   as transportMode : String(40),
        bridge.network         as network       : String(80),
...
union all
  select from BridgeRestrictions {
    key ID,
        'BridgeRestrictions'   as sourceMaster  : String(20),
        bridge.ID              as bridge_ID     : Integer,
        bridge.bridgeId        as bridgeId      : String(40),
        bridge.bridgeName      as bridgeName    : String(111),
        // BridgeRestrictions carries its own mode/network; fall back to the bridge
