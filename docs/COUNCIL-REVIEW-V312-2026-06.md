...
|---|---|---|---|
| Domain / ISO 55000 / NSW | not-top-1% | 3 | Engine plumbing excellent, but user-type math is anti-monotone and diverges from the approved prioritisation specification, fleet rank renormalises away the consequence axis, and BSI applies NHVR road load-rating weights to rail/pedestrian bridges. |
...
| B7 | **User-type axis math does not follow the approved prioritisation specification** — weighted-mean factor is anti-monotone (more customer types lowers priority), AT 0.5 weighting self-cancels, Over/Under is dead code so Under rows hit every bridge. *(Domain, IT)* | Per-type additive scoring with monotone normalisation; apply weighting outside the denominator; derive and pass overUnder; golden-vector tests. |
...
- **Two headline methodologies wouldn't survive an external review.** The customer-type weighting can *lower* a bridge's priority when it serves more user groups (the opposite of the approved specification's intent), and rail/pedestrian bridge health is computed with weight sets the source calculator never intended for those modes.
